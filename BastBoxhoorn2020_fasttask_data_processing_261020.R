...
# from MIND data - data is provided upon reasonable request: nico.bast@kgu.de
  ...
  #data retention:
  #raw data sets: n=97
  #after merging with demographics: n=90
  #--> possible trials 90 participants * 152 trials = 13680
...
setwd(paste(home_path,'/PowerFolders/data_MIND/tasks',sep=''))
...
print(paste('total raw data sets: n =',as.character(length(list_data)))) 
...
print(paste('total raw data sets: n =',as.character(with(df_data,length(unique(id.label)))))) #97
...
#number of possible trials: n=97 * (80 trials baseline x 72 fast task trials): 14744
table(trial.valid)
...
df_demo<-read.csv(paste(home_path,'/PowerFolders/data_MIND/descriptive_data/ParticipantsMiND.CSV',sep=''),sep=';',header = T)
...
length(table(droplevels(df_data$id.label))) #after demographics data merging: n=92

# D. MERGE WITH QUESTIONNAIRE DATA (MICE imputed) ####

#load data from multiple imputated data set (MICE)
load(file=paste(home_path,'/PowerFolders/data_MIND/descriptive_data/MiND_MIdata.Rdata',sep=''))
df_quest<-all.new.data[[41]] #last data frame - i.e. latest iteration in MICE
#get ID labels from final data frame
load(file=paste(home_path,'/PowerFolders/data_MIND/descriptive_data/MiNDfinalsample.RData',sep=''))
...
#--> possible trials: 90 * 152 trials = 13680

# E. MERGE WITH LOGFILES (data retention information, correct trials etc.) #### 
## - read report files ####
...
df_logfile$id.label<-droplevels(df_logfile$id.label)
df_logfile$game_type<-droplevels(df_logfile$game_type)

df_logfile<-df_logfile[df_logfile$id.label %in% levels(df_data$id.label),]

table(df_logfile$trial)
id_logfile<-droplevels(with(df_logfile,interaction(id.label,game_type,trial))) #trial identifier ID x type x trial

df_logfile<-df_logfile[!duplicated(id_logfile),] #remove duplicates
#--> nrow(df_logfile) == number of possible trials: 90 * 152
...
## - CHECK logfile file data (relevant to DATA PROCESSING) ####

attach(df_logfile)
table(target_index,chosen_index) #--> overall performance

hist(reaction_time[reaction_time<1000])
round(table(reaction_time<1500 & reaction_time>200) / sum(table(reaction_time<1500 & reaction_time>200)),4) #correct trials
...
round(table(correct_response) / sum(table(correct_response)),4)
round(table(correct_response,game_type) / colSums(table(correct_response,game_type)),4)
...
## - CREATE row identifier by interaction of id.label and currentTrial in log and report data ####
  ...
  df_logfile$id_logfile<-droplevels(df_logfile$id_logfile)

## - CALCULATE HIT/OMISSION/COMISSION errors (df_logfile) ####
...
pd_list<-lapply(df_split,func_PD_basicpreprocessing)
  ...
  with(df,round(table(!is.na(pd)) / sum(table(is.na(pd))),4)) #valid PD data
  ...
  pd_trial_baseline<-rep(pd_trial_baseline,times=trial.length)
...
# H. DATA CLEANING / RELABELLING ####
  ...
  ##- ::CREATE:: aggregated file (df_describe) ####
  df_valid<-df[df$gaze_valid,]
  aggregate_by<-list(with(df_valid,id.label))
  
  factors_agg<-with(df_valid,aggregate(data.frame(groupASD,groupADHD,id.label,Diagnosis,sex),
                                       by=aggregate_by,head,n=1))
  
  metrics_mean_agg<-with(df_valid,aggregate(data.frame(tpd,pd,rpd,reaction_time,gaze_valid,omission,comission,hit,age,pIQ,vIQ,Sum_FSK_all,srs.sum.all,sum.aufm.all,sum.hypimp.all,CBCL_T_GES_all,RBSR_Bodfish_Ges_all),
                                            by=aggregate_by,mean,na.rm=T))
  
  rt_sd<-as.numeric(with(df_valid,by(reaction_time,aggregate_by,sd,na.rm=T)))
  
  df_describe<-cbind(factors_agg,metrics_mean_agg,rt_sd)
  df_describe<-subset(df_describe,select=-c(Group.1))
  df_describe<-subset(df_describe,select=-c(Group.1)) #run command two times to erase Group.1 variable
  names(df_describe)<-c('groupASD','groupADHD','id.label','Diagnosis','sex','tpd','pd','rpd','rt','gaze_valid','omission','comission','hit','age','pIQ','vIQ','FSK_sum','SRS_sum','INATT_sum','HYP_sum','CBCL_t','RBSR_sum','rt_sd')
  
  
  ##- merge df_describe to df_logfile ####
  
  df_describe_merge<-df_describe[,names(df_describe) %in% c('id.label','groupASD','groupADHD')]
  df_logfile<-merge(df_logfile,df_describe,by='id.label')
  
  df_logfile<-df_logfile[,!(names(df_logfile) %in% c("omission.y",'comission.y','hit.y'))]
  names(df_logfile)[10:12]<-c('hit','omission','comission')
  
  df_logfile<-df_logfile[unique(df_logfile$id_logfile),] #exclude duplicate trials
  df_logfile$id.label<-droplevels(df_logfile$id.label)
...
## --> save relevant object (df) ####
  ...
  #--> save with R 3.6 processed - no embedded null warnings for file reads
  #save(list=c('df','df_describe','df_agg'),file='C:/Users/Nico/PowerFolders/Paper_MINDdata_fasttask/df_021019.Rdata')
  save(list=c('df','df_describe','df_logfile'),file=paste(home_path,'/PowerFolders/Paper_MINDdata_fasttask/df_261020.Rdata',sep=''))
