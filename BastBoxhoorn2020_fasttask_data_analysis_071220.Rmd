...
(ref:figure-caption-2) placeholder figure 2

```{r print_placeholder_figure_2, echo=FALSE, fig.cap="(ref:figure-caption-2)"}
...
path_figure3 <- paste(getwd(),'/output/figures/figure3_DV_distribution.tiff',sep='')
...
(ref:figure-caption-3) Gaussian density distribution of the main dependent variables BPS, SEPR, MRT.

```{r plot_figure2_DV_distribution, echo=FALSE, fig.width=10, fig.height=8, fig.cap="(ref:figure-caption-3)"}
...
#predicted values
  ...
  g4<-ggplot(lmm_contrast_data,aes(x=interaction(group3,typeGame)))+
    geom_jitter(data=df_plot_predicted_RT,
...
#   g4<-set_palette(g4,palette = "Pastel1") #change color palette
  ...
  path_figure4 <- paste(getwd(),'/output/figures/figure4_RT_groups.tiff',sep='')
...
(ref:figure-caption-4) Mean reaction time (MRT) between groups. Slower RT in ASD compared to TD is observed in the low-utility condition, while there is an effect of task condition with faster RT in the high-utility condition. Boxplot whiskers represent 95% confidence intervals. Interquartile range is estimated by marginalized means +/- 1.5 standard errors.

```{r plot_figure2_PD, echo=FALSE, fig.width=7, fig.height=3.5, fig.cap="(ref:figure-caption-4)"}
...
rt_model<-lmer(scale(rt_m)~rpd_phasic_z*rpd_fore_z*typeGame+group3+(1|id.label)+(1|currentTrial),data=df_trial)
...
```{r calculate_figure3_RTbyPD}

path_figure5 <- paste(getwd(),'/output/figures/figure5_arousal_regulation_of_reaction_time.tiff',sep='')
...
path_figure6 <- paste(getwd(),'/output/figures/figure6_arousal_change_byGroup.tiff',sep='')

tiff(filename = path_figure6, 
...
(ref:figure-caption-6) Arousal Change from the low- to the high-utility condition. Boxplot whiskers represent 95% confidence intervals. Interquartile range is estimated by marginalized means +/- 1.5 standard errors.

```{r plot_figure3, fig.cap="(ref:figure-caption-6)", fig.height=3, fig.width=7}
...
rt_model<-lmer(scale(rt_m)~rpd_fore_z*rpd_phasic_z*typeGame+group3+(1|id.label)+(1|currentTrial),data=df_trial)
...
row.names(lmm_rt_data)<-c('BPS','SEPR','condition','group','BPS x SEPR','BPS x condition','SEPR x condition','BPS x SEPR x condition')
